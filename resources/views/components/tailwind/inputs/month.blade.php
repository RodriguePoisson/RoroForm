<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
    ...
    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}"
               class="roro-label roro-label-month block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 {{$labelClass}}">
            {{ $label }}
            @if($required)
