<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
    ...
    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}"
               class="roro-label roro-label-week block text-gray-700 font-semibold mb-2 {{$labelClass}}">
            {{ $label }}
            @if($required)
    ...
    <x-roro-border-error :hidden="!$error || !$enableError">
        <div class="relative">
            <input
                ...
                {{ $attributes->class([
                    'roro-input roro-input-week peer w-full border border-gray-300 bg-white px-4 py-3 text-gray-800 placeholder-transparent focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:outline-none sm:text-sm md:text-base lg:text-lg transition-all duration-200',
                    $class,
                ]) }}
            >
        </div>
