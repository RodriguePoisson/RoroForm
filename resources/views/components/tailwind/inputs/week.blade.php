<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
    ...
    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}"
               class="roro-label roro-label-week block text-sm font-medium text-gray-700 mb-1.5 {{$labelClass}}">
            {{ $label }}
            @if($required)
    ...
    <x-roro-border-error :hidden="!$error || !$enableError">
        <div class="relative">
            <input
                ...
                {{ $attributes->class([
                    'roro-input roro-input-week block w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-500',
                    $class,
                ]) }}
            >
        </div>
