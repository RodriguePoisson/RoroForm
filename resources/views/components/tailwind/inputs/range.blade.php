<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
    ...
    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}" class="roro-label roro-label-range block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 {{$labelClass}}">
            {{ $label }} @if($required)
    ...
    <x-roro-border-error :hidden="!$error || !$enableError">
        <div class="flex items-center gap-4 w-full">
            <input
                ...
                {{ $attributes->class([
                    'roro-input roro-input-range w-full accent-indigo-600 cursor-pointer',
                    $class,
                ]) }}
                oninput="this.nextElementSibling.innerText = this.value"
            >
            <span class="min-w-[40px] text-zinc-700 dark:text-zinc-300 font-semibold">{{ $value }}</span>
        </div>
    </x-roro-border-error>
