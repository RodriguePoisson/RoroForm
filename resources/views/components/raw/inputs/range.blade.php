<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
     class="roro-wrapper roro-wrapper-range @if($hasTopMargins) roro-mt @endif {{$wrapperClass}}">

    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}" class="roro-label roro-label-range {{$labelClass}}">
            {{ $label }} @if($required)<x-roro-required-label></x-roro-required-label>@endif
        </label>
    @endif

    <x-roro-border-error :hidden="!$error || !$enableError">
        <div class="roro-range-wrapper">
            <input
                type="range"
                name="{{ $name }}"
                id="{{ $id }}"
                ...
                {{ $attributes->class([
                    'roro-input roro-input-range',
                    $class,
                ]) }}
                oninput="this.nextElementSibling.innerText=this.value"
            >
            <span class="roro-range-value">{{$value}}</span>
        </div>
    </x-roro-border-error>
