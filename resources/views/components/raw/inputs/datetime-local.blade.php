<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
     class="roro-wrapper roro-wrapper-datetime-local @if($hasTopMargins) roro-mt @endif {{$wrapperClass}}">

    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}" class="roro-label roro-label-datetime-local {{$labelClass}}">
    ...
    <x-roro-border-error :hidden="!$error || !$enableError">
        <input
            ...
            {{ $attributes->class([
                'roro-input roro-input-datetime-local',
                $class,
            ]) }}
        >
    </x-roro-border-error>
