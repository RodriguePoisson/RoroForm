<div id="roro-wrapper-{{$id}}" style="@if($hidden) display:none; @endif"
     class="roro-wrapper roro-wrapper-week w-100 {{$wrapperClass}} @if($hasTopMargins) mt-4 @endif">

    @if($label)
        <label id="label-{{$id}}" for="{{ $id }}"
               class="roro-label roro-label-week form-label {{$labelClass}}">
            {{ $label }}
            @if($required)
    ...
    <x-roro-border-error :hidden="!$error || !$enableError">
        <div class="position-relative">
            <input
                ...
                {{ $attributes->class([
                    'roro-input roro-input-week form-control',
                    $class,
                ]) }}
            >
        </div>
